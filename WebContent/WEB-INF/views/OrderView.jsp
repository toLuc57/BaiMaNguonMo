...
<body>
	...
	<jsp:include page="_menu.jsp"></jsp:include>
      ...
      <form method="POST" action="${pageContext.request.contextPath}/order">
         <input type="hidden" name="idKhachHang" value="${loginedUser.id}"> 
         <table border="0">
         <tr>
         <td>NGÀY</td>
         <td><input type="date" id="orderDate" name="orderDate" value="2010-01-01"></td>
         		...
         		<td>
         			<c:forEach items="${dsccn.keySet()}" var="cn">
         				...
         				<c:forEach items="${dsccn.get(cn)}" var="i">
         					<input type="radio" id="store${i.id}" value="${i.id}" 
         					name="store" onclick="selectStore(${i.id})"> 
         					<label for="store${i.id}">${i.ten}</label>
         				</c:forEach>
         			</div>
         			</c:forEach>
         		</td>
                   ...
                   <c:if test="${not empty dscnvtct.get(cnvt)}">
                     <div style="display:none;" id="${cnvt}" class="dscnvtt">
                       <c:forEach items="${dscnvtct.get(cnvt)}" var="i">
                         <input type="radio" id="staff${i.id}" name="staff" 
                         value="${i.id}" onclick="selectStaff(${i.id})">
                         <label for="staff${i.id}">${i.hoTen}</label>
                         <br/>
                       </c:forEach>
                     </div>
                     <br/>
		...
		<c:forEach items="${dscclvct.keySet()}" var="t">
		 ...
		 <c:forEach items="${dscclvccnv.keySet()}" var="nv">
	      <c:forEach items="${dscclvccnv.get(nv)}" var="i">
		    <input type="hidden" class="${nv}_${i.ngayLamViec}" value="${i.idKhungGio}"> 
	       </c:forEach>
		 </c:forEach>
      </form>	
<script>
...
function selectStaff(idStaff){
	var orderDate = document.getElementById("orderDate").value;
	var displayShifts = document.getElementsByClassName("displayShifts");
	var shiftsStaff = document.getElementsByClassName("dsclvcnv" + idStaff + "_" + orderDate);
	for(let i = 0; i < displayShifts.length; ++i){
		displayShifts[i].style.display = "none";
	}
	for(let j = 0; j < shiftsStaff.length;++j){
		document.getElementById("shifts" + shiftsStaff[j].value).style.display = "block";
	}
}
</script>
</body>
